...
def main():
    ...
    for setup_indx in range(len(number_of_nodes_list)):
        ...
        n_workers = 12
        threads_per_worker = 12 #Not implemnetd yet 16  # 144 cores / 9 workers    
        
        cluster = SLURMCluster(
            processes=n_workers,
            cores=n_workers*threads_per_worker,
            interface='ib0',
            memory="240GB",
        ...
        cluster.scale(cores_per_node * number_of_nodes) 
        viper_client.wait_for_workers(n_workers=cores_per_node * number_of_nodes)
        print("**************")
        print(cluster.job_script())
